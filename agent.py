...
def status():
    ...
    return jsonify({
        ...
        "vps_user": getpass.getuser(),
        "user": (os.environ.get("VPN_USER") or os.environ.get("VPN_USERNAME") or os.environ.get("OPENVPN_USER") or os.environ.get("OPENVPN_USERNAME") or os.environ.get("WG_USER") or getpass.getuser()),
        "vpn_user": (os.environ.get("VPN_USER") or os.environ.get("VPN_USERNAME") or os.environ.get("OPENVPN_USER") or os.environ.get("OPENVPN_USERNAME") or os.environ.get("WG_USER") or ""),
...
# =========================
# ROUTES - EMAILS (NEW)
# index.html butuh endpoint /emails agar bisa baca email.txt dari agent.
# =========================
@app.route("/emails", methods=["GET"])
def emails():
    ensure_files()
    txt = safe_read_text(EMAIL_FILE)
    emails = []
    for ln in (txt or "").splitlines():
        ...
        if not s or s.startswith("#"):
            continue
        emails.append(s)
    return jsonify({"emails": emails, "count": len(emails), "time": _now()})

# compat raw
@app.route("/email.txt", methods=["GET"])
def email_txt():
    ensure_files()
    return Response(safe_read_text(EMAIL_FILE) or "", mimetype="text/plain")
...
<style>
  :root{ --bg:#020617; --panel:#0b1220; --border:#1f2937; --text:#e2e8f0; --muted:#94a3b8; --accent:#38bdf8; }
  [data-theme="light"]{ --bg:#f8fafc; --panel:#ffffff; --border:#e2e8f0; --text:#0f172a; --muted:#475569; --accent:#0284c7; }
  ...
  <title>JOKO Panel</title>
</head>
<body class="tap" data-theme="dark">
  <div class="max-w-6xl mx-auto p-4 md:p-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div>
        <div class="flex items-center gap-2"><div class="text-2xl font-bold">JOKO Panel</div><div class="text-xs px-2 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-200">PRO</div><button id="btnTheme" class="ml-2 px-3 py-2 rounded-xl panel border border-slate-800 hover:bg-slate-800 smooth">🌙</button></div>
        <div class="muted text-sm">No auth key • Local-only • Access via Tunnel</div>
      </div>
      <div class="flex gap-2 flex-wrap">
        <button id="btnRefresh" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Refresh</button>
        <a id="btnLogs" href="/view_logs" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">View Logs</a>
        <a id="btnOpenLive" href="/live" target="_blank" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Open Live</a>
      </div>
    </div>

    <!-- Status Cards -->
    <div class="grid md:grid-cols-4 gap-3 mt-5">
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">LOGIN</div>
        <div id="stLogin" class="text-xl font-semibold">-</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">LOOP</div>
        <div id="stLoop" class="text-xl font-semibold">-</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">BUAT_LINK</div>
        <div id="stBuatLink" class="text-xl font-semibold">-</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">noVNC READY</div>
        <div id="stNoVNC" class="text-xl font-semibold">-</div>
      </div>
    </div>
    ...
    <div class="mt-4 grid md:grid-cols-4 gap-3">
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">CPU</div>
        <div class="text-xl font-semibold"><span id="cpuPct">-</span>%</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">RAM</div>
        <div class="text-sm text-slate-300">Used: <b><span id="ramUsed">-</span> MB</b></div>
        <div class="text-sm text-slate-300">Free: <b><span id="ramFree">-</span> MB</b></div>
        <div class="text-xs muted"><span id="ramPct">-</span>% of <span id="ramTotal">-</span> MB</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">Disk</div>
        <div class="text-sm text-slate-300">Used: <b><span id="diskUsedGb">-</span> GB</b></div>
        <div class="text-xs muted"><span id="diskPct">-</span>% of <span id="diskTotalGb">-</span> GB</div>
      </div>
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="muted text-xs">Chrome / Profiles</div>
        <div class="text-sm text-slate-300">Profiles dir:</div>
        <div id="profilesDir" class="text-xs muted break-all">-</div>
      </div>
    </div>

    <!-- Tunnel -->
    <div class="mt-4 p-4 rounded-2xl panel border border-slate-800">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <div class="font-semibold">🌐 Tunnel</div>
          <div class="muted text-sm">URL akan auto refresh kalau tunnel jalan.</div>
          <div class="mt-2 text-sm">
            <span class="muted">Tunnel URL:</span>
            <a id="tunnelUrl" class="ml-2 underline text-sky-300 break-all" href="#" target="_blank">-</a>
          </div>
        </div>
        <div class="flex flex-wrap gap-2">
          <button id="btnStartTunnelLogin" class="px-3 py-2 rounded-xl bg-sky-700 hover:bg-sky-600">Start Tunnel + Login</button>
          <button id="btnStopTunnel" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Stop Tunnel</button>
        </div>
      </div>
    </div>
    ...
    <div class="mt-4 p-4 rounded-2xl panel border border-slate-800">
      <div class="flex items-center justify-between">
        <div>
          <div class="font-semibold">🧾 Login Monitor</div>
          <div class="muted text-sm">Realtime dari login_log.txt</div>
        </div>
        <button id="btnLoginMonRefresh" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Refresh</button>
      </div>
      <div class="mt-3 grid md:grid-cols-3 gap-3">
        <div class="p-3 rounded-2xl bg-black/30 border border-slate-800">
          <div class="text-xs muted">Current</div>
          <div id="loginCur" class="text-sm text-slate-200 break-all">-</div>
        </div>
        <div class="p-3 rounded-2xl bg-black/30 border border-slate-800">
          <div class="text-xs muted">Status</div>
          <div id="loginSt" class="text-sm text-slate-200">-</div>
        </div>
        <div class="p-3 rounded-2xl bg-black/30 border border-slate-800">
          <div class="text-xs muted">Last line</div>
          <div id="loginLast" class="text-sm text-slate-200 break-all">-</div>
        </div>
      </div>
      <pre id="loginTail" class="mt-3 p-3 rounded-2xl bg-black/40 border border-slate-800 text-xs overflow-auto max-h-56 whitespace-pre-wrap">-</pre>
    </div>
    ...
    <div class="mt-4 p-4 rounded-2xl panel border border-slate-800">
      <div class="flex items-center justify-between">
        <div>
          <div class="font-semibold">📡 Loop Monitor per Profile</div>
          <div class="muted text-sm">Realtime progress dari loop.py</div>
        </div>
        <button id="btnProfilesRefresh" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Refresh</button>
      </div>

      <div id="profilesBox" class="mt-3 grid md:grid-cols-3 gap-3">
...
<div class="mt-4 grid md:grid-cols-2 gap-3">
  <div class="p-4 rounded-2xl panel border soft-shadow">
    <div class="flex items-center justify-between">
      <div>
        <div class="text-sm muted">CPU</div>
        <div class="text-2xl font-bold"><span id="cpuVal">-</span>%</div>
      </div>
      <div class="text-xs muted text-right">
        <div>Load: <span id="loadVal">-</span></div>
        <div>Uptime: <span id="uptimeVal">-</span></div>
      </div>
    </div>
    <div class="mt-3">
      <canvas id="cpuChart" width="800" height="220"></canvas>
    </div>
  </div>
  ...
  <div class="p-4 rounded-2xl panel border soft-shadow">
    <div class="flex items-center justify-between">
      <div>
        <div class="text-sm muted">RAM</div>
        <div class="text-2xl font-bold"><span id="ramVal">-</span>%</div>
      </div>
      <div class="text-xs muted text-right">
        <div>Used: <span id="ramUsed">-</span> GB</div>
        <div>Total: <span id="ramTotal">-</span> GB</div>
      </div>
    </div>
    <div class="mt-3">
      <canvas id="ramChart" width="800" height="220"></canvas>
    </div>
  </div>
</div>
    ...
    <div class="mt-4 grid md:grid-cols-3 gap-3">
      <div class="p-4 rounded-2xl panel border border-slate-800">
        <div class="font-semibold">▶️ Controls</div>
        <div class="mt-3 flex flex-wrap gap-2">
          <button id="btnStartLogin" class="px-3 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-600">Start Login</button>
          <button id="btnStopLogin" class="px-3 py-2 rounded-xl bg-rose-700 hover:bg-rose-600">Stop Login</button>

          <button id="btnStartLoop" class="px-3 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-600">Start Loop</button>
          <button id="btnStopLoop" class="px-3 py-2 rounded-xl bg-rose-700 hover:bg-rose-600">Stop Loop</button>

          <button id="btnStartBuatLink" class="px-3 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-600">Start Buat Link</button>
          <button id="btnStopBuatLink" class="px-3 py-2 rounded-xl bg-rose-700 hover:bg-rose-600">Stop Buat Link</button>

          <button id="btnStopAll" class="px-3 py-2 rounded-xl bg-rose-800 hover:bg-rose-700">Stop ALL</button>
          <button id="btnKillAll" class="px-3 py-2 rounded-xl bg-rose-900 hover:bg-rose-800">Kill ALL Proses</button>
          <button id="btnClearRam" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Clear RAM Cache</button>
        </div>

        <div class="mt-4 pt-4 border-t border-slate-800">
          <div class="font-semibold">♻️ Reset Data</div>
          <div class="mt-3 flex flex-wrap gap-2">
            <button id="btnResetMapping" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Reset Mapping</button>
            <button id="btnResetProfiles" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Reset Chrome Profiles</button>
          </div>
          <div class="mt-2 text-xs muted">Reset Profiles akan stop semua proses + close Chrome dulu.</div>
        </div>
      </div>

      <div class="p-4 rounded-2xl panel border border-slate-800 md:col-span-2">
        <div class="font-semibold">🖼️ Screenshots</div>
        <div class="mt-3 flex flex-wrap gap-2">
    ...
    <!-- File Manager -->
    <div class="mt-4 p-4 rounded-2xl panel border border-slate-800">
      <div class="font-semibold">🗂️ File Manager</div>
      <div class="muted text-sm">List semua file di folder JOKO (recursive). Bisa read / edit / delete.</div>

      <div class="mt-3 flex flex-col md:flex-row gap-2">
        <button id="btnFilesRefresh" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Refresh Files</button>
        <input id="fileName" class="flex-1 px-3 py-2 rounded-xl bg-black/40 border border-slate-800" placeholder="contoh: email.txt atau subfolder/file.txt" />
        <button id="btnFileRead" class="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">Read</button>
        <button id="btnFileSave" class="px-3 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-600">Save</button>
        <button id="btnFileDelete" class="px-3 py-2 rounded-xl bg-rose-700 hover:bg-rose-600">Delete</button>
      </div>

      <div class="mt-3 grid md:grid-cols-2 gap-3">
        <div class="p-3 rounded-xl bg-black/40 border border-slate-800">
          <div class="text-xs muted mb-2">Files</div>
          <div id="filesList" class="text-sm space-y-1 max-h-64 overflow-auto"></div>
        </div>
        <div class="p-3 rounded-xl bg-black/40 border border-slate-800">
          <div class="text-xs muted mb-2">Editor</div>
          <textarea id="fileContent" class="w-full h-64 p-3 rounded-xl bg-black/30 border border-slate-800 text-xs"></textarea>
        </div>
      </div>
    </div>

    <div class="mt-4 text-xs muted">
      base_dir: <span id="baseDir">-</span> • screenshots: <span id="screensDir">-</span> • time: <span id="timeNow">-</span>
    </div>
  </div>
...
function drawLineChart(canvasId, values, label){
  const c = $(canvasId);
  if(!c) return;
  ...
  ctx.beginPath();
  for(let i=0;i<n;i++){
    const v = Math.min(maxV, Math.max(minV, values[i]));
    const x = pad + dx*i;
...
function profileCard(p){
  ...
  const color = state.includes("ERROR") ? "border-rose-700/60" : (state.includes("RUN") ? "border-emerald-700/50" : "border-slate-800");
  return `
    <div class="p-3 rounded-2xl bg-black/30 border ${color}">
      <div class="flex items-center justify-between">
        <div class="font-semibold">${p.name || "-"}</div>
        <div class="text-xs muted">${state}</div>
      </div>
      <div class="mt-2 text-xs text-slate-300">
        Round: <b>${round}</b> • Link: <b>${idx}/${total}</b>
      </div>
      <div class="mt-2 text-xs break-all muted">${link}</div>
      ${ws ? `<div class="mt-2 text-xs text-sky-300 break-all">Workspace: <b>${ws}</b></div>` : ``}
      ${err ? `<div class="mt-2 text-xs text-rose-300 break-all">Err: ${err}</div>` : ``}
      <div class="mt-2 text-[11px] muted">Update: ${last}</div>
    </div>
  `;
}

async function refreshProfiles(){
  const r = await api("/loop/profiles");
  const box = $("profilesBox");
  const items = r.profiles || [];
  if(items.length === 0){
    box.innerHTML = `<div class="muted text-sm">Belum ada data. Jalankan loop.py dulu.</div>`;
    return;
  }
...
async function refreshLoginMonitor(){
  const r = await api("/login/monitor");
  const m = r.monitor || {};
  $("loginCur").textContent = m.current || "-";
  $("loginSt").textContent = m.status || "-";
  $("loginLast").textContent = m.last_line || "-";
  $("loginTail").textContent = r.tail || "-";
}

async function refreshScreens(){
  const r = await api("/screens/list");
  const box = $("screensBox");
  box.innerHTML = "";
  (r.screens || []).slice(0, 24).forEach(it => {
    const a = document.createElement("a");
    a.href = "/screens/view?name=" + encodeURIComponent(it.name);
    a.target = "_blank";
    a.className = "block p-2 rounded-xl bg-black/40 border border-slate-800 hover:border-slate-600";
    a.innerHTML = `<div class="text-xs break-all">${it.name}</div>`;
    box.appendChild(a);
  });
}
...
async function refreshFiles(){
  ...
  (r.files || []).forEach(fn => {
    ...
    b.className = "block w-full text-left px-2 py-1 rounded-lg hover:bg-slate-800/60";
    b.textContent = fn;
    b.onclick = () => { $("fileName").value = fn; };
    list.appendChild(b);
  });
}
...
async function fileRead(){
  ...
  const r = await api("/files/read", "POST", { name });
...
async function fileSave(){
  ...
  if(!name) return alert("Isi nama file dulu");
  const r = await api("/files/write", "POST", { name, content });
  if(r.error) return alert(r.error);
  await refreshFiles();
  alert("Saved: " + name);
}
...
async function fileDelete(){
  ...
  const r = await api("/files/delete", "POST", { name, recursive: true });
  if(r.error) return alert(r.error);
  $("fileContent").value = "";
  await refreshFiles();
  alert("Deleted: " + name);
...
$("btnRefresh").onclick = async ()=>{ await refreshStatus(); await refreshLoginMonitor(); await refreshProfiles(); };
...
$("btnKillAll").onclick = async ()=>{
  if(!confirm("KILL ALL proses? (login/loop/buat_link/chrome/xvfb)")) return;
  const r = await api("/kill/all","POST");
  alert(r.msg || r.error || "done");
  await refreshStatus();
};
$("btnClearRam").onclick = async ()=>{
  if(!confirm("Clear RAM cache (drop_caches)?")) return;
  const r = await api("/system/clear_ram","POST");
  alert(r.msg || r.error || "done");
  await refreshStatus();
};
